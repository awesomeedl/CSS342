...
{
  clear();
  theSquares = new Square[other.currentSize];
  currentSize = other.currentSize;
  nextItem = other.nextItem;
  copy(this->theSquares, other.theSquares);
  return *this;
}

// Stream operator overload
ostream& operator<<(ostream &Out, const SquareContainer &sc) {
  for(int i = 0; i < sc.nextItem; i++)
  {
    Out << "Size of square " << i << ": " << sc.theSquares[i].getSize() << endl;
  }
  return Out;
}